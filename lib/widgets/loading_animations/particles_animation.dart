...
class ParticlesAnimation extends LoadingAnimation {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < _particleCount; i++) {
      final seed = i * 73 + 19;
      final sx = 0.15 + (seed % 11) * 0.04;
      final sy = 0.12 + (seed % 9) * 0.045;
      final px = seed * 1.1;
      final py = seed * 0.9;
    ...
    for (var i = 0; i < _particleCount; i++) {
      for (var j = i + 1; j < _particleCount; j++) {
        ...
        if (dist < _connectionDistance) {
          final alpha = (1.0 - dist / _connectionDistance) * 0.3;
    ...
    for (var i = 0; i < _particleCount; i++) {
      final color = colors[i % colors.length];
      // 呼吸するようにサイズが変わる
      final breathe = 1.0 + 0.3 * sin(elapsed * 1.5 + i * 0.5);
      final radius = 2.0 * breathe;
